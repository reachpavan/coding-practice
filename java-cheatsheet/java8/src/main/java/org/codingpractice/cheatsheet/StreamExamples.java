...
public class StreamExamples {
  ...
  private static void mapExamples() {
    ...
    List<Integer> ints =
        Arrays.stream(s.split("\\s+")).map(Integer::parseInt).collect(Collectors.toList());
    ...
    // Everywhere else where you have to pass a function or a predicate, you can use one of the
    // three forms:
  ...
  private static void filterExamples() {
    List<String> words =
        Arrays.asList("tst", "one", "two", "three", "four", "five", "rotator", "deified");
  ...
  private static void aggregatingOperationsExamples() {
    List<String> words =
        Arrays.stream("Once upon a midnight dreary while I pondered weak and weary".split("\\s+"))
            .collect(Collectors.toList());
  ...
  private static void moreFunnyExamples() {
    List<String> words =
        Arrays.stream("These examples cover only a small part of Java 8 Streams.".split("\\s+"))
            .collect(Collectors.toList());
    ...
    String withDelimiter =
        words.stream().map(it -> Integer.toString(it.length())).collect(Collectors.joining(" + "));
    System.out.println(withDelimiter);

    System.out.println("2. Concat two streams:");
    List<String> concat =
        Stream.concat(
                words.stream(), words.stream().map(s -> new StringBuilder(s).reverse().toString()))
            .collect(Collectors.toList());
    System.out.println(concat);

    System.out.println("3. Create a Map from stream");
    // toMap takes two arguments: the first is the function to get keys (we pass identity there --
    // it is a function
    ...
    List<Integer> part =
        Stream.of(1, 2, 3, 4, 5, 7, 8, 9).skip(2).limit(4).collect(Collectors.toList());
    ...
    // `reduce` is an operation with an accumulator: we provide an initial value for the accumulator
    // and a function
    // with two arguments. As one of the arguments the current accumulator value will always be
    // passed.
    // The function is applied to every item in the stream and its result is the next value of the
    // accumulator.
    //
    // Now let's try to implement ints production which, unfortunately, is not there along with
    // `sum` and `average`.
    int production = Stream.of(1, 2, 3, 5, 7, 11).reduce(1, (acc, i) -> acc * i);
    System.out.println(production);

    System.out.println("8. Int range");
    // Yeah, `mapToObj(i -> i)` looks weird, but in Java IntStream and Stream are not the same, and
    // this is the
    ...
    // Flat map is similar to map: it iterates over every item in the stream and calls the function
    // which you
    // pass as the argument. But your function should return not a single value but a stream. And
    // flat map then
    // concatenates all the streams returned.
    List<Character> chars =
        words.stream().flatMap(w -> w.chars().mapToObj(c -> (char) c)).collect(Collectors.toList());
    System.out.println(chars);

    System.out.println("10. Group by");
    // The result is a map where for each value of the function passed to groupingBy there is a list
    // of the stream
